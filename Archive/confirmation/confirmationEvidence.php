...
$sql = "CREATE TABLE IF NOT EXISTS confirmation_evidence (
  id INT (100) UNSIGNED
  AUTO_INCREMENT,
...
$sql = "SELECT * FROM confirmation_evidence WHERE book_number = '$book_number' && book_page = '$book_page'";
$result = mysqli_query($con, $sql);
if ($result && mysqli_num_rows($result) == 0) {
  $sql = "INSERT INTO confirmation_evidence (book_number, book_page) VALUES (?, ?)";
...
$sql = "SELECT * FROM confirmation_evidence WHERE book_number = '$book_number' AND book_page = '$book_page'";
$result = mysqli_query($con, $sql);
...
$upload_dir = '../../images/Evidence/confirmation/';



if (isset($_FILES['confirmation-evidence'])) {
  $book_number = $_POST['book-number'];
  $book_page = $_POST['book-page'];

  // Handle file upload
  $uploaded_files = [];
  $files = $_FILES['confirmation-evidence'];
  ...
  $sql = "UPDATE confirmation_evidence SET image1 = ? WHERE book_number = ? AND book_page = ?";
  $stmt = $con->prepare($sql);
...
$sql = "SELECT image1 FROM confirmation_evidence WHERE book_number = ? AND book_page = ?";
$stmt = $con->prepare($sql);
$stmt->bind_param("ss", $book_number, $book_page);
$stmt->execute();
...
<body id="evidenceBody">
  ...
  <header class="evidenceHeading">
    <h1 class="align-self-center">CONFIRMATION EVIDENCE <br> Book <span><?php echo $book_number; ?></span> Page <span><?php echo $book_page; ?></span> </h1>
  </header>
    ...
    if ($noImage) { ?>
      <div class="drop_box">
        ...
        <form method="post" class="d-flex justify-content-center align-items-center fileContainer" enctype="multipart/form-data">
          ...
          <input name="confirmation-evidence[]" type="file" id="fileID" class="form-control" style="position:absolute; width:126px; padding:10px 0; opacity:0" multiple>
