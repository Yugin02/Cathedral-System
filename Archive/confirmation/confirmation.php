    ...
    if ($error === 0) {
      if ($imageSize < 500000) {
        ...
        if (move_uploaded_file($imageTmp, $folder)) {
          $sql = "update `confirmation` set child_baptismal_image = '$imageNew_name' where Child_name = '$Child_name'";
...
<body>
  ...
  <section class="px-2 py-3">
    ...
    <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]); ?>" method="post" class="d-flex align-items-center gap-2 page" style="margin-bottom: 10px;">
      ...
      <button name="subtract" <?php if ($value <= 1) echo "disabled"; ?>><svg xmlns="http://www.w3.org/2000/svg" width="25" height="25" fill="currentColor" class="bi bi-arrow-left-short" viewBox="0 0 16 16">
          <path fill-rule="evenodd" d="M12 8a.5.5 0 0 1-.5.5H5.707l2.147 2.146a.5.5 0 0 1-.708.708l-3-3a.5.5 0 0 1 0-.708l3-3a.5.5 0 1 1 .708.708L5.707 7.5H11.5a.5.5 0 0 1 .5.5" />
        </svg></button>
      <input style="display:none" type="text" name="value1" value="<?php echo $value; ?>" disabled>
      <input type="text" style="width: 5%; text-align:center; align-self:stretch" name="value2" value="<?php echo $value; ?>">
      <button name="add"><svg xmlns="http://www.w3.org/2000/svg" width="25" height="25" fill="currentColor" class="bi bi-arrow-right-short" viewBox="0 0 16 16">
  ...
  if (isset($_GET["id"])) {
    ...
    $sql1 = "SELECT * from `confirmation` where id='$baptismal_id'";
    $result1 = mysqli_query($con, $sql1);
    $row1 = mysqli_fetch_assoc($result1);
    $Child_name = $row1['Child_name'];
    $Child_familyname = $row1['Child_familyname'];
    $baptismal_name = $row1['child_baptismal_image'];
  ?>
    <?php
    echo
    ...
    <table class=\"table text-center table-hover\" style=\"font-size: 14px !important;\">
      ...
      <tbody>
        <tr>
          <td style = \" padding:10px;\">$baptismal_name</td>
    ...
    <div class=\"d-flex flex-column align-items-end gap-3\" style= \"cursor:pointer;\">
      ...
      <div id=\"imageButton\" class=\"d-flex flex-column gap-1 align-items-start\">
        ...
        <form id=\"uploadForm\" action=\"confirmation.php\" method=\"POST\" enctype=\"multipart/form-data\">
            <input type=\"hidden\" name=\"childName\" value=\"$Child_name\" />
            <input type=\"hidden\" name=\"childFamilyName\" value=\"$Child_familyname\" />
    ...
    <div class=\"d-flex justify-content-center\" style=\"height: 90%; width:95%; align-self:center;\">
      <img style=\"width:100%; object-fit:contain; object-position:center\" src=\"../../images/Confirmation/$baptismal_name\">
    </div>
  </div>"
    ?>
  <?php
