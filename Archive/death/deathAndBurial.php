...
<head>
  ...
  <link rel="stylesheet" href="../css/deathAndBurial.css">
...
<body>
  ...
  <section class="px-2 py-3">
    <div class="d-flex justify-content-between my-4">
      ...
      <button style=" color:black; border: solid 1px black; padding:0; margin:0;" type="button" class="add_data_button btn btn-info"><a style="border:none; padding: 15px 20px; text-decoration:none; color:black; font-weight:600" href="deathAndBurialAddData.php">Add Data</a></button>
    </div>
    ...
    <div class="table-responsive">
      <table class="table text-center table-responsive table-hover align-middle" style="background: #88CAD6; border:1px solid #000; box-shadow: 0px 4px 4px 0px rgba(0, 0, 0, 0.25); font-size: 13px; width:100vw; min-width: 95rem">
        <thead class="table-primary align-middle" style="border: 1px solid #000;">
          <tr style="border: 1px solid #000;">
            ...
            <th colspan="1" style="background-color: #88CAD6;">
              <p>PLACE AND DATE OF BURIAL</p>
            </th>
            <th colspan="1" style="background-color: #88CAD6;">
              <p>SACRAMENTS <br>RECEIVED</p>
            </th>
            <th colspan="1" style="background-color: #88CAD6;">
              <p>MINISTER</p>
            </th>
        ...
        <tbody>
          ...
          if (isset($_POST['search1'])) {
            ...
            $sql = "SELECT * FROM `death_and_burial` 
            ...
            if ($result) {
              if (mysqli_num_rows($result) > 0) {
                while ($row = mysqli_fetch_assoc($result)) {
                  ...
                  echo '<tr>
                      <td >' . $row['Book_line'] . '</td>
                      <td >' . $row['deceased_name'] . ' ' . $row['deceased_familyname'] . '</td>
                      <td >' . $row['deceased_municipality'] . ' ' . $row['deceased_barangay'] . '</td>
                      <td >' . $row['age'] . '</td>
                      <td >' . $row['relative_name'] . ' ' . $row['relative_familyname'] . '</td>
                      <td >' . $row['death_year'] . ' ' . $row['death_month'] . ' ' . $row['death_day'] . '</td>
                      <td ><div>' . $row['burial_municipality'] . ' ' . $row['burial_barangay'] . '</div><div style="border-top:1px solid #000;">' . $row['burial_year'] . ' ' . $row['burial_month'] . ' ' . $row['burial_day'] . '</div></td>
                      <td >' . $row['sacraments'] . '</td>
                      <td >' . $row['minister'] . '</td>';
          ?>
                  <td style="cursor:pointer;">
                    <form class="image_form" action="deathAndBurial.php" method="get">
  ...
  if (isset($_GET["id"])) {
    $death_id = $_GET["id"];
    $sql1 = "SELECT * from `death_and_burial` where id='$death_id'";
    $result1 = mysqli_query($con, $sql1);
    $row1 = mysqli_fetch_assoc($result1);
    $deceased_name = $row1['deceased_name'];
    $deceased_familyname = $row1['deceased_familyname'];
    $death_cert_name = $row1['death_cert_images'];
  ?>
    <?php
    echo
    "<div class=\"p-3 d-flex flex-column\" id=\"uploadFile\">
    <h3>Upload Section</h3>
    <p>Name: <span>$deceased_name $deceased_familyname</span> </p>
    <table class=\"table text-center table-hover\" style=\"font-size: 14px !important;\">
      ...
      <tbody>
        <tr>
          <td style = \" padding:10px;\">$death_cert_name</td>
