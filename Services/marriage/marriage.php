    ...
    <table class="table table-responsive table-hover align-middle" style="background: #88CAD6;
      ...
      <thead class="table-primary align-middle">
        <tr>
          ...
          <th scope="col" style="background-color: #88CAD6;">Date of Marriage</th>
      ...
      <tbody>
        ...
        if (isset($_POST['search1'])) {
          ...
          $sql = "SELECT *, CONCAT_WS(' ', wife_name, wife_familyname,) AS fullname, CONCAT_WS(' ', marriage_month, marriage_day, marriage_year) AS date FROM `marriage` 
          WHERE wife_name LIKE '%$search%' 
          OR wife_familyname LIKE '%$search%' 
          OR husband_name LIKE '%$search%' 
          OR husband_familyname LIKE '%$search%' 
          OR marriage_month LIKE '%$search%' 
          OR marriage_day LIKE '%$search%' 
          OR marriage_year LIKE '%$search%'
          ...
          if ($result) {
            if (mysqli_num_rows($result) > 0) {
              while ($row = mysqli_fetch_assoc($result)) {
                ...
                echo '
              <tr>
              <td onclick="window.location=\'marriageCert.php?certid=' . $id . '\';">' . $row['id_number'] . '</td>
              <td onclick="window.location=\'marriageCert.php?certid=' . $id . '\';">Mr.&Mrs. ' . $row['fullname'] . '</td>
              <td onclick="window.location=\'marriageCert.php?certid=' . $id . '\';">' . $row['date'] . '</td>
              <td>
                <button class="btn btn-info" type="button"><a class="edit" style="text-decoration: none; color: #000;" href="marriageEdit.php?editid=' . $id . '">Edit</a></button>
