...
<body>
    ...
    <table class="table table-responsive table-hover align-middle" style="background: #88CAD6;
      ...
      <thead class="table-primary align-middle">
        <tr>
          ...
          <th scope="col" style="background-color: #88CAD6;">Death Date</th>
      ...
      <tbody>
        ...
        if (isset($_POST['search1'])) {
          ...
          $sql = "SELECT *, CONCAT_WS(' ', deceased_name, deceased_familyname) AS fullname, CONCAT_WS('/', death_month, death_day, death_year) AS date
          FROM `death_and_burial` 
          WHERE deceased_name LIKE '%$search%' 
          OR deceased_familyname LIKE '%$search%'
          OR death_month LIKE '%$search%' 
          OR death_day LIKE '%$search%' 
          OR death_year LIKE '%$search%'
          ...
          if ($result) {
            if (mysqli_num_rows($result) > 0) {
              while ($row = mysqli_fetch_assoc($result)) {
                ...
                echo '
              <tr>
              <td onclick="window.location=\'deathAndBurialCert.php?certid=' . $id . '\';">' . $row['id_number'] . '</td>
              <td onclick="window.location=\'deathAndBurialCert.php?certid=' . $id . '\';">' . $row['fullname'] . '</td>
              <td onclick="window.location=\'deathAndBurialCert.php?certid=' . $id . '\';">' . $row['date'] . '</td>
              <td>
                <button class="btn btn-info" type="button"><a class="edit" style="text-decoration: none; color: #000;" href="deathAndBurialEdit.php?editid=' . $id . '">Edit</a></button>
  ...
  <script src="deathAndBurial.js"></script>
</body>
