...
<body>
  ...
  <nav class="container-fluid services">
    <ul>
    ...
    <table class="table table-responsive table-hover align-middle" style="background: #88CAD6;
      ...
      <thead class="table-primary align-middle">
        <tr>
          ...
          <th scope="col" style="background-color: #88CAD6;">Date Baptized</th>
      ...
      <tbody>
        ...
        if (isset($_POST['search1'])) {
          ...
          if ($result) {
            if (mysqli_num_rows($result) > 0) {
              while ($row = mysqli_fetch_assoc($result)) {
                ...
                echo '
              <tr>
              <td onclick="window.location=\'BaptismalCert.php?certid=' . $id . '\';">' . $row['id_number'] . '</td>
              <td onclick="window.location=\'BaptismalCert.php?certid=' . $id . '\';">' . $row['fullname'] . '</td>
              <td onclick="window.location=\'BaptismalCert.php?certid=' . $id . '\';">' . $row['date'] . '</td>
              <td>
                <button class="btn btn-info" type="button"><a class="edit" style="text-decoration: none; color: #000;" href="BaptismalEdit.php?editid=' . $id . '">Edit</a></button>
          ...
          $sql = "SELECT *, CONCAT(Child_name, ' ', Child_familyname) AS fullname, CONCAT_WS(' ', baptism_month, baptism_day, baptism_year) as date FROM `baptismal`";
          $result = mysqli_query($con, $sql);
          if ($result) {
            while ($row = mysqli_fetch_assoc($result)) {
              $id = $row['id'];
              ...
              echo '<tr>
                  <td onclick="window.location=\'BaptismalCert.php?certid=' . $id . '\';">' . $id_number . '</td>
                  <td onclick="window.location=\'BaptismalCert.php?certid=' . $id . '\';">' . $fullname . '</td>
                  <td onclick="window.location=\'BaptismalCert.php?certid=' . $id . '\';">' . $date . '</td>
                  <td>
                    <button class="btn btn-info" type="button"><a class="edit" style="text-decoration: none; color: #000;" href="BaptismalEdit.php?editid=' . $id . '">Edit</a></button>
                    <button class="btn btn-danger" type="button"><a class="delete" style="text-decoration: none; color: #000;" onclick="confirmDelete(' . $id . ')">Delete</a></button>
                  </td>
        ...
        <div class="d-flex flex-column gap-3">
          ...
          <div>
            ...
            <div class="d-flex align-items-center gap-2" style="padding-left: 10px;">
              ...
              <a href=""><svg xmlns="http://www.w3.org/2000/svg" width="30" height="30" viewBox="0 0 45 45" fill="none">
                  <g clip-path="url(#clip0_1224_1521)">
                    <path d="M34.4531 0H10.5469C4.722 0 0 4.722 0 10.5469V34.4531C0 40.278 4.722 45 10.5469 45H34.4531C40.278 45 45 40.278 45 34.4531V10.5469C45 4.722 40.278 0 34.4531 0Z" fill="white" />
                    <path d="M34.4531 0H10.5469C4.722 0 0 4.722 0 10.5469V34.4531C0 40.278 4.722 45 10.5469 45H34.4531C40.278 45 45 40.278 45 34.4531V10.5469C45 4.722 40.278 0 34.4531 0Z" fill="#1D9BF0" />
                    <path d="M35.081 16.0683C35.1004 16.3473 35.1004 16.6263 35.1004 16.9079C35.1004 25.4855 28.5704 35.3781 16.6301 35.3781V35.3728C13.1028 35.3779 9.64846 34.3677 6.67969 32.4627C7.19262 32.5244 7.70801 32.5552 8.2248 32.5564C11.1484 32.5588 13.9879 31.5782 16.287 29.7722C14.9324 29.7467 13.6197 29.2984 12.5323 28.4902C11.4449 27.6819 10.6373 26.5541 10.2224 25.2643C11.1949 25.4518 12.1976 25.4136 13.153 25.1525C10.1246 24.5406 7.94584 21.8798 7.94584 18.7896V18.7073C8.84864 19.2102 9.85901 19.4888 10.8919 19.5198C8.03971 17.6134 7.16045 13.8189 8.88275 10.852C10.5125 12.8575 12.5458 14.4977 14.8508 15.6661C17.1557 16.8346 19.6807 17.505 22.2616 17.634C22.0043 16.5263 22.0423 15.3705 22.3719 14.2821C22.7014 13.1936 23.3109 12.2108 24.1395 11.4319C26.7541 8.97415 30.8661 9.10019 33.3239 11.7134C34.7776 11.4263 36.1717 10.8934 37.4462 10.1375C36.9615 11.6405 35.9474 12.9163 34.5925 13.7276C35.8794 13.576 37.136 13.2316 38.3203 12.7058C37.4492 14.0105 36.3522 15.149 35.081 16.0683Z" fill="white" />
                  </g>
                  <defs>
                    <clipPath id="clip0_1224_1521">
                      <rect width="45" height="45" fill="white" />
                    </clipPath>
                  </defs>
                </svg></a>
